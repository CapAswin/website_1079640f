...
function handleFile(file) {
  ...
  if (!isCsv && !isExcel) {
    alert("Please upload CSV or Excel file (.csv, .xlsx, .xls).");
    return;
  }
  ...
  parseFileByType(file)
    .then((results) => {
      ...
      if (state.rawRows.length === 0) {
        ...
        warningsEl.textContent = "No data rows found in this file.";
      }
    })
    .catch((error) => {
